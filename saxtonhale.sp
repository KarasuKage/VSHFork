...
//Gentlespy Models
#define GentleModel "models/freak_fortress_2/gentlespy/the_gentlespy_v1.mdl"
static const String:GentleMats[][] = {
    "materials/freak_fortress_2/gentlespy_tex/stylish_spy_blue.vmt",
    "materials/freak_fortress_2/gentlespy_tex/stylish_spy_blue.vtf",
    "materials/freak_fortress_2/gentlespy_tex/stylish_spy_blue_invun.vmt",
    ...
    "materials/freak_fortress_2/gentlespy_tex/stylish_spy_red.vmt",
...
// SFX
#define GentleTheme "freak_fortress_2/gentlespy/gentle_music.mp3"
#define GentleKSpree1              "vo/spy_specialcompleted11.mp3"
#define GentleKSpree2              "vo/taunts/spy_taunts06.mp3"
#define GentleKSpree3              "vo/spy_specialcompleted-assistedkill02.mp3"
#define GentleWin 				"vo/taunts/spy_highfive_success03.mp3"
#define GentleFail1 "vo/spy_revenge03.mp3"
#define GentleFail2 "vo/taunts/spy_taunts10.mp3"
#define GentleRage "vo/spy_specialcompleted01.mp3"
#define GentleRoundStart1 "vo/spy_revenge03.mp3"
#define GentleRoundStart2 "vo/taunts/spy_taunts10.mp3"
// Saxton Hale Files
...
AddToDownload()
{
    ...
    */
	//GentleSpy
	///Models First
	PrepareModel(GentleModel);
	DownloadMaterialList(GentleMats, sizeof(GentleMats)); // Materials List for gentles models

	PrepareSound(GentleTheme);
    PrecacheSound(GentleKSpree1, true);
    PrecacheSound(GentleKSpree2, true);
    PrecacheSound(GentleKSpree3, true);
    PrecacheSound(GentleWin, true);
    PrecacheSound(GentleFail1, true);
    PrecacheSound(GentleFail2, true);
    PrecacheSound(GentleRage, true);
    PrecacheSound(GentleRoundStart1, true);
    PrecacheSound(GentleRoundStart2, true);
  // Saxton Hale
...
public CvarChange(Handle:convar, const String:oldValue[], const String:newValue[])
{
    ...
    else if (convar == cvarEnabled)
    {
        if (GetConVarBool(convar) && IsSaxtonHaleMap())
        {
            ...
            {
                decl String:gameDesc[64];
                Format(gameDesc, sizeof(gameDesc), "VS Saxton Hale (%s) Edited", haleversiontitles[maxversion]);
                Steam_SetGameDescription(gameDesc);
            }
#endif
        }
...
#endif
				case 5: Incoming = VSHSpecial_Gentle;
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
...
#endif
			case VSHSpecial_Gentle:
			{
				Format(s, PLATFORM_MAX_PATH, "%s", GentleWin);
                EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
                EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, Hale, _, NULL_VECTOR, false, 0.0);
			}
        }
    ...
    ///since multiple themes are giving me a headache logic wise
    ClearTimer(MusicTimer);
    if (IsClientInGame(Hale))
    {
        ...
        if (IsPlayerAlive(Hale))
        {
            ...
            switch (Special)
            {
                ...
                case VSHSpecial_Gentle:     strcopy(translation, sizeof(translation), "vsh_gentle_is_alive");
    ...
    switch (Special)
    {
        ...
        case VSHSpecial_Gentle:
        {
            ...
            switch(rand)
            {
            	case 1:
            	{
            		strcopy(s, PLATFORM_MAX_PATH, GentleRoundStart1);
            	}
            	case 2:
            	{
            		strcopy(s, PLATFORM_MAX_PATH, GentleRoundStart2);
            	}
...
public Action:StartHaleTimer(Handle:hTimer)
{
    ...
    ///got no idea if i should do this but meh what the heck
    //defalt is the normal hale thing others are for changing the round.
	switch(Special)
	{
		default:
		{
...
//          }
            ...
            case VSHSpecial_Gentle:
            {
                strcopy(sound, sizeof(sound), GentleTheme);
                time = 137.0;
            }
        }
    }
...
public Action:MessageTimer(Handle:hTimer, any:allclients)
{
    ...
    switch (Special)
    {
        ...
        case VSHSpecial_Gentle: strcopy(translation, sizeof(translation), "vsh_start_gentle");
...
EquipSaxton(client)
{
    ...
    switch (Special)
    {
        ...
        case VSHSpecial_Gentle:
        {
            SaxtonWeapon = SpawnWeapon(client, "tf_weapon_knife", 225, 100, TFQual_Unusual, "68 ; 2.0 ; 2 ; 3.1 ; 259 ; 1.0");
            //SetEntProp(client, Prop_Send, "m_nBody", 0);
        }
        default:
        {
...
public Action:Command_GetHP(client)
{
    ...
    if (client == Hale)
    {
        switch (Special)
        {
            ...
            case VSHSpecial_Gentle:
                PriorityCenterTextAll(_, "%t", "vsh_gentle_show_hp", HaleHealth, HaleHealthMax);
            default:
    ...
    if (IsNextTime(e_flNextHealthQuery)) //  GetGameTime() >= HPTime
    {
        ...
        switch (Special)
        {
            ...
            case VSHSpecial_Gentle:
            {
                PriorityCenterTextAll(_, "%t", "vsh_gentle_hp", HaleHealth, HaleHealthMax);
                CPrintToChatAll("{olive}[VSH]{default} %t", "vsh_gentle_hp", HaleHealth, HaleHealthMax);
            }
            default:
            {
...
public Action:Command_MakeNextSpecial(client, args)
{
    ...
    else if (StrContains(arg, "gentle", false) != -1)
    {
        Incoming = VSHSpecial_Gentle;
        name = "GentleSpy";
    }
#if defined EASTER_BUNNY_ON
...
public Action:HaleTimer(Handle:hTimer)
{
	...
	switch(SlowestClass)
	{
		...
		{
			speed = DemoSpeed + 45.0;
		}
		case TFClass_Heavy:
		{
			speed = HeavySpeed + 45.0;
		}
		case TFClass_Spy:
		{
			speed = SpySpeed + 45.0;
		...
		case TFClass_Medic:
		{
			speed = MedicSpeed + 45.0;
		}
		case TFClass_Pyro:
		{
			speed = PyroSpeed + 45.0;
		}
		case TFClass_Scout:
		{
			speed = ScoutSpeed + 45.0;
		}
		case TFClass_Engineer:
		{
			speed = EngineerSpeed + 45.0;
		}
		case TFClass_Soldier:
		{
			speed = SoldierSpeed + 45.0;
		}
	}
 //   new Float:speed = HaleSpeed + 0.7 * (100 - HaleHealth * 100 / HaleHealthMax);
    ...
    else if (HaleCharge < 0)
    {
		switch(Special)
		{
			...
			{
				HaleCharge += 5;
				if (!(GetClientButtons(Hale) & IN_SCORE)) ShowSyncHudText(Hale, jumpHUD, "%t %i", "vsh_jump_status_2", -HaleCharge/20);
			}
		}
    }
    else
    {
        ...
        if ((ang[0] < -45.0) && (HaleCharge > 1))
        {
			...
			switch(Special)
			{
				...
				case VSHSpecial_HHH:
				{
					if(HaleCharge == HALEHHH_TELEPORTCHARGE || bEnableSuperDuperJump)
					{
						new target = -1;
				...
				///special cases where bosses have no teleport or super jump
				case VSHSpecial_Gentle:
				{
...
//              SetEntPropEnt(weapon, Prop_Send, "m_hExtraWearableViewModel", vm);
            ...
            case VSHSpecial_CBS:
            {
                ...
                EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
                TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
                SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", SpawnWeapon(client, "tf_weapon_compound_bow", 1005, 100, 5, "2 ; 2.1 ; 6 ; 0.5 ; 37 ; 0.0 ; 280 ; 19 ; 551 ; 1 ; 258 ; 1"));
            ...
            case VSHSpecial_Gentle:
            {
                Format(s, PLATFORM_MAX_PATH, "%s", GentleRage);
                EmitSoundToAll(s, _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, _, pos, NULL_VECTOR, false, 0.0);
  				TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
  				new weapon = SpawnWeapon(client, "tf_weapon_revolver", 61, 100, 5, "2 ; 9 ; 51 ; 1 ; 309 ; 1 ; 391 ; 99 ; 258 ; 1");
                SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon); 
                SetEntProp(weapon, Prop_Send, "m_iClip1", 4);
                SetAmmo(client, TFWeaponSlot_Primary, 0);
                TF2_AddCondition(Hale, TFCond_UberchargedHidden, 4.0);
                ...
                if (IsValidClient(target)) // Maybe it can fail it we teleport to nobody?
                {
                    ...
                    AttachParticle(Hale, "ghost_appearation", 3.0);    
                    if (TeleMeToYou(Hale, target)) // This returns true if teleport to a ducking player happened
                    {
                        VSHFlags[Hale] |= VSHFLAG_NEEDSTODUCK;
...
//              CreateTimer(0.1, Timer_ChangeRagdoll, any:GetEventInt(event, "userid"));
			...
			case VSHSpecial_Gentle:
			{
				...
				switch(rand)
				{
					case 1:
					{
 						Format(s, PLATFORM_MAX_PATH, "%s", GentleFail1);
                		EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);
               			EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);
					}
					case 2:
					{
						Format(s, PLATFORM_MAX_PATH, "%s", GentleFail2);
                		EmitSoundToAll(s, _, SNDCHAN_VOICE, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);
               			EmitSoundToAllExcept(SOUNDEXCEPT_VOICE, s, _, SNDCHAN_ITEM, SNDLEVEL_TRAFFIC, SND_NOFLAGS, SNDVOL_NORMAL, 100, client, NULL_VECTOR, NULL_VECTOR, false, 0.0);	
					}
				}
            ...
            switch (Special)
            {
				...
				case VSHSpecial_Gentle:
				{
					...
					switch(rand)
					{
						case 0:
						{
							Format(s, PLATFORM_MAX_PATH, GentleKSpree1);
						}
						case 1:
						{
							Format(s, PLATFORM_MAX_PATH, GentleKSpree2);
						}	
						case 2:
						{
							Format(s, PLATFORM_MAX_PATH, GentleKSpree3);
						}							
					}
				}
            }
...
public Action:CheckAlivePlayers(Handle:hTimer)
{
    ...
    if (Special == VSHSpecial_CBS || Special == VSHSpecial_Gentle && GetAmmo(Hale, 0) > RedAlivePlayers && RedAlivePlayers != 0) SetAmmo(Hale, 0, RedAlivePlayers);
    ...
    else if (RedAlivePlayers == 1 && IsValidClient(Hale) && VSHRoundState == VSHRState_Active)
    {
		...
		switch(Special)
		{
			case VSHSpecial_HHH,VSHSpecial_Gentle:
			{
...
public Action:HintPanel(client)
{
    ...
    switch (Special)
    {
        ...
        case VSHSpecial_Gentle:
            Format(s, 512, "%t", "vsh_help_gentle");
    }
...
stock GetSlowPlayer()
{
	for (new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && IsPlayerAlive(i))
		{
			///since idk what order switch statements pings 
			//one long if chain to ensure the order
			/// i can probably do this better but cant be asked for now 
			new class = TF2_GetPlayerClass(i);
			...
			else if(class == TFClass_Sniper || TFClass_Engineer || TFClass_Spy || TFClass_Pyro)
			{
				return class;
			}
	...
	///if for some reason ALL OF THAT FAILS 
	return -1;
}
